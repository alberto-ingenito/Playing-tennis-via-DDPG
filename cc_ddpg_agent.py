...
class Agents():
    ...
    def act(self, states, add_noise=True):
        ...
        self.actor_local.eval() # By default, train mode is on. Let's disable it to use evaluation/inference mode
        
        with torch.no_grad():
            actions = self.actor_local(states).cpu().data.numpy()

        self.actor_local.train() # Restore train mode
        if add_noise:
    ...
    def load_to_cpu(self, path):
        device = torch.device('cpu')
        self.actor_local.load_state_dict(torch.load(path,map_location='cpu'))
        self.actor_local.eval()
class OUNoise:
